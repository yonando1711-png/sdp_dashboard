                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 font-medium">
                    ...
                    @forelse($items as $index => $item)
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                            ...
                            <td class="py-4 px-5">
                                @if(!empty($item->bbn))
                                    <span class="font-semibold text-slate-800 dark:text-slate-200">{{ $item->bbn }}</span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-500 text-[11px] font-bold border border-amber-500/20" title="BBN not specified in Odoo">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                                        No BBN on Odoo
                                    </span>
                                @endif
