...
@section('content')
    <div x-data="{
        ...
        activeTab: (window.location.hash === '#crm') ? 'crm' : ((window.location.hash === '#lor') ? 'lor' : ((window.location.hash === '#surat-kuasa' || window.location.hash === '#suratkuasa') ? 'surat-kuasa' : ((window.location.hash === '#disposal') ? 'disposal' : 'excel'))),
        odooConfig: {
            url: '{{ $odooConfig['url'] ?? '' }}',
            db: '{{ $odooConfig['db'] ?? '' }}',
        ...
        <div x-show="!isUnlocked"
            ...
            <div
                class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl p-8 w-full max-w-md mx-4 border border-slate-200 dark:border-slate-700">
                ...
                <form @submit.prevent="checkPassword()">
                    <div class="mb-4">
                        <input type="password" x-model="passwordInput" placeholder="Enter password"
                            ...
                            autofocus>
                        <p x-show="passwordError" class="text-red-500 text-sm mt-2 text-center">Incorrect password. Please
                            try again.</p>
                    </div>
                ...
                <div class="mt-6 text-center">
                    <a href="{{ route('dashboard') }}"
                        class="text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 text-sm transition-colors">
                        ← Back to Dashboard
                    </a>
                </div>
            </div>
        ...
        <div x-show="isUnlocked" x-transition>
            ...
            <div class="mb-8">
                ...
                <p class="text-slate-500 dark:text-slate-400 mt-2">Import inventory data from Excel or sync directly from
                    Odoo</p>
            </div>
            ...
            <div
                ...
                <div class="flex border-b border-slate-200 dark:border-slate-700">
                    ...
                    <button @click="activeTab = 'lor'"
                        :class="activeTab === 'lor' ? 'border-b-2 border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'"
                        class="flex items-center gap-2 px-6 py-4 font-medium transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z">
                    ...
                    <button @click="activeTab = 'disposal'"
                        :class="activeTab === 'disposal' ? 'border-b-2 border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'"
                        class="flex items-center gap-2 px-6 py-4 font-medium transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z">
                ...
                <div class="p-6">
                    ...
                    <div x-show="activeTab === 'excel'" x-transition>
                        <div class="max-w-xl">
                            ...
                            <p class="text-slate-500 dark:text-slate-400 mb-6">Upload your inventory Excel file (.xlsx,
                                .xls, .csv) to import data into the dashboard.</p>

                            <form action="{{ route('import.excel') }}" method="POST" enctype="multipart/form-data"
                                x-data="{ isUploading: false, fileName: '' }" @submit="isUploading = true">
                                @csrf

                                <div
                                    class="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl p-8 text-center hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors">
                                    <svg class="w-12 h-12 mx-auto text-slate-400 dark:text-slate-500 mb-4" fill="none"
                                    ...
                                    <label class="cursor-pointer">
                                        <span
                                            class="text-indigo-600 dark:text-indigo-400 font-medium hover:text-indigo-700 dark:hover:text-indigo-300">Choose
                                            file</span>
                                    ...
                                    <p class="text-xs text-slate-400 dark:text-slate-500 mt-2">XLSX, XLS, or CSV up to 10MB
                                    </p>

                                    <p x-show="fileName" x-text="fileName"
                                        class="mt-3 text-sm text-indigo-600 dark:text-indigo-400 font-medium"></p>
                                </div>
                                ...
                                <button type="submit"
                                    ...
                                    <svg x-show="isUploading" class="animate-spin h-5 w-5"
                    ...
                    <div x-show="activeTab === 'odoo'" x-transition>
                        <div class="max-w-xl">
                            <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">Odoo Configuration
                            </h3>
                            <p class="text-slate-500 dark:text-slate-400 mb-6">Configure your Odoo connection to sync
                                inventory data directly.</p>

                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Odoo
                                        URL</label>
                                    <input type="url" x-model="odooConfig.url" placeholder="https://your-odoo-instance.com"
                                ...
                                <div>
                                    <label
                                        class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Database</label>
                                    <input type="text" x-model="odooConfig.db" placeholder="your-database-name"
                                        class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
                                </div>

                                <div>
                                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">User
                                        Email</label>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">API Key
                                        / Password</label>
                        ...
                        <div x-show="testResult" x-transition class="mt-4">
                            <div :class="testResult?.success ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800 text-red-700 dark:text-red-400'"
                                ...
                                <svg x-show="testResult?.success" class="w-5 h-5 flex-shrink-0 mt-0.5" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div x-show="syncResult" x-transition class="mt-4">
                            <div :class="syncResult?.success ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800 text-red-700 dark:text-red-400'"
                                class="p-4 rounded-lg border flex items-start gap-3">
                                <svg x-show="syncResult?.success" class="w-5 h-5 flex-shrink-0 mt-0.5" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                        ...
                        <div class="mt-8 border-t border-slate-200 dark:border-slate-700 pt-8">
                            ...
                            <div class="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-5 space-y-4">
                                ...
                                <div class="flex items-center justify-between">
                                    <div>
                                        <label class="text-sm font-medium text-slate-700 dark:text-slate-300">Enable
                                            Auto-Sync</label>
                                        <p class="text-xs text-slate-500 dark:text-slate-400">Automatically sync data from
                                            Odoo</p>
                                    </div>
                                ...
                                <div x-show="schedule.enabled" x-transition>
                                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Sync
                                        Interval</label>
                                ...
                                <!-- Last Sync Info -->
                                <div
                                    class="flex items-center justify-between text-sm pt-2 border-t border-slate-200 dark:border-slate-700">
                                    <span class="text-slate-500 dark:text-slate-400">Last Sync:</span>
                                    <span class="text-slate-700 dark:text-slate-300 font-medium"
                                        x-text="formatDate(schedule.lastSync)"></span>
                                </div>
                                ...
                                <button @click="saveSchedule()" :disabled="isSavingSchedule"
                                    ...
                                    <svg x-show="isSavingSchedule" class="animate-spin h-4 w-4" fill="none"
                                        viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
                                            stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor"
                        ...
                        <!-- Info Box -->
                        <div
                            class="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-lg">
                            ...
                            <p class="text-sm text-blue-600 dark:text-blue-300">
                                The Odoo sync requires PHP's <code
                                    class="px-1 py-0.5 bg-blue-100 dark:bg-blue-800 rounded">xmlrpc</code> extension.
                ...
                <div x-show="activeTab === 'history'" x-transition>
                    <div class="max-w-4xl">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <button @click="loadHistory()" :disabled="isLoadingHistory"
                                ...
                                <svg :class="isLoadingHistory ? 'animate-spin' : ''" class="w-4 h-4" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <!-- Empty State -->
                        <div x-show="!isLoadingHistory && historyData.length === 0"
                            class="text-center py-12 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                            <svg class="w-12 h-12 mx-auto text-slate-400 dark:text-slate-500 mb-4" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z">
                            ...
                            <p class="text-sm text-slate-400 dark:text-slate-500 mt-1">Import data via Excel or Odoo to see
                                history here</p>
                        </div>
                        ...
                        <div x-show="!isLoadingHistory && historyData.length > 0"
                                        ...
                                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                            <td class="px-4 py-3">
                                                <span class="text-sm text-slate-700 dark:text-slate-300"
                                                    x-text="formatDate(log.imported_at)"></span>
                                            </td>
                                            <td class="px-4 py-3">
                                                <span
                                                    ...
                                                    <svg x-show="log.source === 'excel'" class="w-3.5 h-3.5" fill="none"
                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                    ...
                                                    <svg x-show="log.source.startsWith('odoo')" class="w-3.5 h-3.5"
                                            ...
                                            <td class="px-4 py-3">
                                                <span
                                                    class="text-sm text-slate-600 dark:text-slate-400 truncate max-w-[200px] block"
                                                    x-text="log.filename || '—'"></span>
                                            </td>
                                            <td class="px-4 py-3 text-right">
                                                <span class="text-sm font-medium text-slate-700 dark:text-slate-300"
                                                    x-text="formatNumber(log.items_count)"></span>
                                            </td>
                                            <td class="px-4 py-3 text-center">
                                                <span
                                                    ...
                                                    <svg x-show="log.status === 'success'" class="w-3 h-3" fill="none"
                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            <td class="px-4 py-3 text-center">
                                                <button @click="toggleExpand(log.id)"
                                                    ...
                                                    <svg class="w-4 h-4 transition-transform"
                                                        :class="expandedLogId === log.id ? 'rotate-180' : ''" fill="none"
                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <template x-for="log in historyData" :key="'summary-' + log.id">
                                <div x-show="expandedLogId === log.id" x-transition:enter="transition ease-out duration-200"
                                    ...
                                    <template x-if="log.summary">
                                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                            <div
                                                class="bg-white dark:bg-slate-900 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
                                                <p class="text-xs text-slate-500 dark:text-slate-400 uppercase">SDP Stock
                                                </p>
                                                <p class="text-lg font-semibold text-slate-800 dark:text-slate-200"
                                                    x-text="formatNumber(log.summary.sdp_stock)"></p>
                                            </div>
                                            <div
                                                class="bg-white dark:bg-slate-900 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
                                                <p class="text-xs text-slate-500 dark:text-slate-400 uppercase">In Stock</p>
                                                <p class="text-lg font-semibold text-slate-800 dark:text-slate-200"
                                                    x-text="formatNumber(log.summary.in_stock?.total)"></p>
                                            </div>
                                            <div
                                                class="bg-white dark:bg-slate-900 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
                                                <p class="text-xs text-slate-500 dark:text-slate-400 uppercase">Rented</p>
                                                <p class="text-lg font-semibold text-slate-800 dark:text-slate-200"
                                                    x-text="formatNumber(log.summary.rented_in_customer?.total)"></p>
                                            </div>
                                            <div
                                                class="bg-white dark:bg-slate-900 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
                                                <p class="text-xs text-slate-500 dark:text-slate-400 uppercase">Vendor Rent
                                                </p>
                                                <p class="text-lg font-semibold text-slate-800 dark:text-slate-200"
                                                    x-text="formatNumber(log.summary.vendor_rent)"></p>
                                            </div>
                                        </div>
                                    </template>
                                </div>
                    ...
                    <div x-show="activeTab === 'crm'" x-transition style="display: none;">
                        <div class="max-w-xl">
                            ...
                            <p class="text-slate-500 dark:text-slate-400 mb-8">Manage the access password for the CRM page.</p>

                            <form action="{{ route('crm.settings.update') }}#crm" method="POST">
                                @csrf
                                <div class="bg-white dark:bg-slate-800/80 rounded-2xl p-8 border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                                    <div class="flex items-center gap-4 mb-6 pb-6 border-b border-slate-100 dark:border-slate-700/50">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Update the master password used to unlock CRM.</p>
                                        </div>
                                    </div>
                    ...
                    <!-- LoR Settings Tab -->
                    <div x-show="activeTab === 'lor'" x-transition style="display: none;">
                        <div class="max-w-xl">
                            <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">LoR Module Settings</h3>
                            <p class="text-slate-500 dark:text-slate-400 mb-8">Manage the access password for the LoR page.</p>

                            <form action="{{ route('lor.settings.update') }}#lor" method="POST">
                                @csrf
                                <div class="bg-white dark:bg-slate-800/80 rounded-2xl p-8 border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                                    <div class="flex items-center gap-4 mb-6 pb-6 border-b border-slate-100 dark:border-slate-700/50">
                                        <div class="p-3 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-xl">
                                        ...
                                        <div>
                                            <h4 class="text-lg font-bold text-slate-800 dark:text-slate-200">Security Access</h4>
                                            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Update the master password used to unlock LoR.</p>
                                        </div>
                                    </div>
                    ...
                    <div x-show="activeTab === 'disposal'" x-transition style="display: none;">
                        <div class="max-w-xl">
                            <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">Disposal Module Settings</h3>
                            <p class="text-slate-500 dark:text-slate-400 mb-8">Manage the secondary access password for the Disposal Fleet Lifecycle page.</p>

                            <form action="{{ route('disposal.settings.update') }}#disposal" method="POST">
                                @csrf
                                <div class="bg-white dark:bg-slate-800/80 rounded-2xl p-8 border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                                    <div class="flex items-center gap-4 mb-6 pb-6 border-b border-slate-100 dark:border-slate-700/50">
                                        ...
                                        <div>
                                            <h4 class="text-lg font-bold text-slate-800 dark:text-slate-200">Security Access</h4>
                                            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Update the master password used to unlock Disposal.</p>
                                        </div>
                                    </div>

                                    <div class="mb-5">
                                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">New Disposal Password</label>
                                        <input type="password" name="password" placeholder="Enter new Disposal password (min. 4 characters)..." required minlength="4"
                                               class="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all shadow-inner">
                                    </div>
                                    <div>
                                        <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-3 px-6 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center gap-2">
